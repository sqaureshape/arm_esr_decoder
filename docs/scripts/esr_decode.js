...
function createFieldBreakdownRows(esr) {
  ...
  // Create the exact format from the reference - using colspan for proper field alignment
  ...
  rows += `<tr>`;
  rows += `<td colspan="32" class="field-value">ISS: 0x${iss.toString(16).toUpperCase()}<br><span class="field-desc">Instruction Specific Syndrome</span></td>`;
  rows += `</tr>`;
...
function addDetailedISSBreakdown(iss, ec) {
  ...
  if (ec === 0x1E) {
    // RME specific fields with proper colspan alignment
    rows += `<tr>`;
    ...
    // Abort specific fields with proper colspan alignment - exactly like reference
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-name">RES0</td>`;
    ...
    // SVE, FP, or BTI abort with proper colspan alignment
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-name">RES0</td>`;
