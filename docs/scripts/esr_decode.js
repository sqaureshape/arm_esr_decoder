...
function createFieldBreakdownRows(esr) {
  ...
  // Create the exact format from the reference - using proper colspan alignment
  
  // EC field (bits 26-31) - 6 bits
  rows += `<tr>`;
  rows += `<td colspan="6" class="field-name">EC</td>`;
  rows += `<td colspan="26" class="field-name">RES0</td>`;
  rows += `</tr>`;
  
  rows += `<tr>`;
  rows += `<td colspan="6" class="field-value">EC: 0x${ec.toString(16).toUpperCase()}<br><span class="field-desc">${getECDescription(ec)}</span></td>`;
  rows += `<td colspan="26" class="field-value">RES0</td>`;
  rows += `</tr>`;
  
  // IL field (bit 25) - 1 bit
  rows += `<tr>`;
  rows += `<td colspan="6" class="field-name">RES0</td>`;
  rows += `<td colspan="1" class="field-name">IL</td>`;
  rows += `<td colspan="25" class="field-name">RES0</td>`;
  rows += `</tr>`;
  
  const il = (esr >> 25) & 0x1;
  rows += `<tr>`;
  rows += `<td colspan="6" class="field-value">RES0</td>`;
  rows += `<td colspan="1" class="field-value">IL: ${il ? 'true' : 'false'}<br><span class="field-desc">${il ? '32-bit instruction trapped' : '16-bit instruction trapped'}</span></td>`;
  rows += `<td colspan="25" class="field-value">RES0</td>`;
  rows += `</tr>`;
  
  // ISS field (bits 0-24) - 25 bits
  rows += `<tr>`;
  rows += `<td colspan="7" class="field-name">RES0</td>`;
  rows += `<td colspan="25" class="field-name">ISS</td>`;
  rows += `</tr>`;
  
  rows += `<tr>`;
  rows += `<td colspan="7" class="field-value">RES0</td>`;
  rows += `<td colspan="25" class="field-value">ISS: 0x${iss.toString(16).toUpperCase()}<br><span class="field-desc">Instruction Specific Syndrome</span></td>`;
  rows += `</tr>`;
...
function addDetailedISSBreakdown(iss, ec) {
  ...
  if (ec === 0x1E) {
    ...
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-name">RES0</td>`;
    rows += `<td colspan="1" class="field-name">GPF</td>`;
    ...
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-value">RES0</td>`;
    rows += `<td colspan="1" class="field-value">GPF: ${gpf ? 'true' : 'false'}<br><span class="field-desc">Granule Protection Fault</span></td>`;
    ...
    // The ISS is bits 0-24, so we need to account for the 7 RES0 bits at the top
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-name">RES0</td>`;
    ...
    rows += `<td colspan="13" class="field-name">Reserved</td>`;
    rows += `</tr>`;
    ...
    rows += `<tr>`;
    rows += `<td colspan="7" class="field-value">RES0</td>`;
    rows += `<td colspan="6" class="field-value">IFSC: 0x${fsc.toString(16).toUpperCase()}<br><span class="field-desc">${getFSCDescription(fsc)}</span></td>`;
    ...
    rows += `<td colspan="13" class="field-value">Reserved: 0x${((iss >> 12) & 0x1FFF).toString(16).toUpperCase()}</td>`;
    rows += `</tr>`;
    ...
    rows += `<td colspan="1" class="field-name">Reserved</td>`;
    rows += `</tr>`;
    ...
    rows += `<td colspan="1" class="field-value">Reserved: 0x00</td>`;
    rows += `</tr>`;
  }
